...
**依赖：** 成员 B 提供路由/布局/API，成员 I 提供可视化数据 API
...
**职责二：可视化数据 API（第 3~4 周）— 代码开发**

| 工作项 | 说明 |
|--------|------|
| 可视化状态 API | `GET /visualization/{ds_type}` — 返回数据结构初始状态数据 |
| 操作步骤 API | `POST /visualization/{ds_type}/step` — 接收操作，返回下一步状态 |
| 数据结构状态生成 | 为每种数据结构实现初始状态生成逻辑（array, linked-list, stack, queue, binary-tree, bst, graph） |
| 操作模拟逻辑 | 模拟插入/删除/遍历等操作的状态变更（纯数据层，不涉及 Canvas 渲染） |
...
.env.example
backend/app/api/v1/visualization.py         # 可视化数据 API 路由
backend/app/services/visualization_service.py # 可视化状态生成与操作模拟
```
...
| **I** | KG 规范 + 汇总 | — | DevOps & 可视化数据 API | Docker, PostgreSQL, Neo4j, FastAPI |
